...
class _SnackState extends State<Snack> {
  ...
  Widget _buildSnack() {
    ...
    if (_foodItem.item3 == "Snack") {
      if (_shouldAdd == true) {
        _addItem(_foodItem);
        widget.foodInfo = const Tuple2(Tuple3('air', 0, "snack"), false);
        _shouldAdd = false;
      }
    }
        ...
        itemBuilder: (context, i) {
          return Card(
            ...
            child: ListTile(
              ...
              title: Text(
                'Meal: ' +
                    _snack[i].item1.toString() +
                    '       Calories: ' +
                    _snack[i].item2.toString(),
                style: _biggerFont,
              ),
              trailing: const Icon(
...
class Snack extends StatefulWidget {
  ...
  Snack({Key? key, required this.foodInfo}) : super(key: key);
